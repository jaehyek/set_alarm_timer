...
class AlarmManagerService extends SystemService {
    ...
    void rebatchAllAlarmsLocked(boolean doValidate) {
        ...
        for (int batchNum = 0; batchNum < oldBatches; batchNum++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (alarm.operation.isCanceled()) {
                    String key = alarm.operation.getCreatorPackage();
        ...
        if (!map.isEmpty()) {
            Slog.e(TAG, "Exist canceled pending intent");
            for (String key : map.keySet()) {
                Slog.e(TAG, "Cancel " + map.get(key) + " alarm(s) in package " + key);
            }
        }
    ...
    void setImpl(int type, long triggerAtTime, long windowLength, long interval,
        ...
        long min_futurity = mConstants.MIN_FUTURITY;
        if (getContext().checkPermission("com.lge.permission.SHORT_ALARM",
    ...
    private final IBinder mService = new IAlarmManager.Stub() {
        ...
        public void set(int type, long triggerAtTime, long windowLength, long interval, int flags,
            ...
            } else if (getContext().checkPermission("com.lge.permission.ALWAYS_EXACT_ALARM",
                Binder.getCallingPid(), Binder.getCallingUid()) == PackageManager.PERMISSION_GRANTED) {
        ...
        public void setTimeZone(String tz) {
            ...
            try {
                Slog.i(TAG, "setTimeZone : " + tz + ", calling pid : " + pid);
                setTimeZoneImpl(tz);
            } finally {
        ...
        for (int i=0; i<triggerList.size(); i++) {
            ...
            try {
                ...
                if (alarm.type == ELAPSED_REALTIME_WAKEUP
                    ...
                    if (Configs.getConfigs(getContext()).logLevel1()) {
                        Slog.v(TAG, labelForType(alarm.type) + " set : " + alarm + " when " + alarm.when);
                    }
                }
            } catch (PendingIntent.CanceledException e) {
                if (alarm.repeatInterval > 0) {
